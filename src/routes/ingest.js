...
async function ingestRoutes(fastify) {
  ...
  fastify.post('/api/ingest/metrics', { preHandler: [requireAgentKey] }, async (req, reply) => {
    ...
    await updateAgentLastSeen(req.agent.id);
    await insertMetric(metrics, req.agent.id);

    const recentLogs = await getLogs(20, null, req.agent.id);
    broadcast(
  ...
  fastify.post('/api/ingest/logs', { preHandler: [requireAgentKey] }, async (req, reply) => {
    ...
    const saved = await insertLog(log, req.agent.id);
    broadcast({ type: 'log', data: saved }, String(req.agent.id));
